...
Future<ContactList> updateContactList(String lastname, String firstname, List<dynamic> contactNumbers, String id) async {
  final res = await http.put(
    Uri.parse('$hostLink/contactNumbers/$id'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
      body: jsonEncode(<dynamic, dynamic>{
...
deleteContactList(String id) async {
...
class _ContactListDBState extends State<ContactListDB> {
  ...
  fetchNumContacts() async{
    final req = await http.get(
        Uri.parse('http://192.168.254.107:3000/contactNumbers/totalCount'));
    continfo = req.body;
    return continfo;
  }
@override
                                ...
                                actions: <Widget>[
                                  ...
                                  FutureBuilder<ContactList>(
                                    builder: (context, snapshot) {
                                      return TextButton(
                                          onPressed: () {
                                            deleteContactList(snapshot.data!.id.toString());
                                            fetchNumContacts().then((result){
                                              setState((){
                                                numContacts = int.parse(result);
                                                for (int i = 0; i < numContacts; i++){
...
class _ForUpdatingContactsState extends State<ForUpdatingContacts> {
  ...
  void initState(){
    super.initState();
    _futureContacts = fetchContactList(widget.index);
  }
  void _saveLatestValue(){
                           ...
                           return ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              setState(() {
                                ...
                                _futureContacts = updateContactList(
                                  _firstNameController.text,
